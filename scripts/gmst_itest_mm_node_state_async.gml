///gmst_itest_mm_node_state_async(phase)
switch (argument0) {
  case TEST_PHASE.SETUP:
    ___current_test_name___ = "Minimax async in node state";
    var _ = -1,
        O = 0,
        X = 1;
  ...
  case TEST_PHASE.RUNNING:
    if (tree_daemon.ready) {
      assert_equal(mm_get_best_move(tree_daemon.tree), 4, "Asynchronous Minimax in node state did not get perfect reply to corner tic-tac-toe opening!");
    }
