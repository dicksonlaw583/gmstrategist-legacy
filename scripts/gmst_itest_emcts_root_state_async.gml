///gmst_itest_emcts_root_state_async(phase)
...
switch (argument0) {
  case TEST_PHASE.SETUP:
    ___current_test_name___ = "eMCTS async in root state";
    states = GmstArray(
  ...
  case TEST_PHASE.RUNNING:
    if (tree_daemon.ready) {
      assert_equal(mcts_get_best_move(tree_daemon.tree), expected_outcomes[current_state], "Asynchronous MCTS in root state did not get perfect counter to die " + chr(ord("A")+current_state) + "!");
